...
public class FileIngestionService : IFileIngestionService
{
    ...
    public Task<IReadOnlyList<TransactionDto>> ParseCsvAsync(PendingFile file, CancellationToken ct = default)
    {
        ...
        var rowNumber = 1;
        using var parser = new TextFieldParser(file.FullPath)
        {
        ...
        if (parser.EndOfData)
            return Task.FromResult<IReadOnlyList<TransactionDto>>(transactions);
        ...
        while (!parser.EndOfData)
        {
            ...
            {
                _logger.LogWarning(
                    "CSV malformed line at row {Row} in {FileName}: {Error}",
                    rowNumber, file.FileName, ex.Message);
                continue;
            }
    ...
    public Task<IReadOnlyList<TransactionDto>> ParseExcelAsync(PendingFile file, CancellationToken ct = default)
    {
        ...
        if (sharedStringsEntry != null)
        {
            ...
            sharedStrings = ReadSharedStrings(stream);
        }
        ...
        var rows = ReadWorksheetRows(worksheetStream, sharedStrings);
        if (rows.Count == 0)
            return Task.FromResult<IReadOnlyList<TransactionDto>>(Array.Empty<TransactionDto>());
    ...
    public Task MoveToProcessedAsync(PendingFile file, CancellationToken ct = default)
    {
        ...
        var destinationPath = MoveFileWithRetry(file.FullPath, _options.ProcessedDirectory, file.FileName);
        _logger.LogInformation(
    ...
    public Task MoveToErrorAsync(PendingFile file, string error, CancellationToken ct = default)
    {
        ...
        var destinationPath = MoveFileWithRetry(file.FullPath, _options.ErrorDirectory, file.FileName);
        var errorTracePath = $"{destinationPath}.error.txt";
        File.WriteAllText(errorTracePath, $"{DateTimeOffset.UtcNow:O}{Environment.NewLine}{error}");
        _logger.LogWarning(
    ...
    private string MoveFileWithRetry(string sourcePath, string destinationDirectory, string originalFileName)
    {
        var attempts = 0;
    ...
    private static List<string[]> ReadWorksheetRows(Stream stream, IReadOnlyList<string> sharedStrings)
    {
        XNamespace ns = "http://schemas.openxmlformats.org/spreadsheetml/2006/main";
        var doc = XDocument.Load(stream);
        var rowElements = doc.Descendants(ns + "sheetData").Elements(ns + "row");
        ...
        foreach (var rowElement in rowElements)
        {
            ...
            foreach (var cell in rowElement.Elements(ns + "c"))
            {
                ...
                valuesByColumn[column] = ReadCellValue(cell, sharedStrings, ns);
            }
